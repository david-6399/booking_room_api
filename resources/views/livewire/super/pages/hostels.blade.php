<div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-4 sm:p-6 border-b border-gray-100">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-4 ">
                                <div class="flex items-center space-x-4">
                                    <button id="open-create-modal"
                                        class="bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center space-x-2" wire:click='addNewHostel()'>
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M12 4v16m8-8H4" />
                                        </svg>
                                        <span class="hidden sm:inline">Add Hostel</span>
                                    </button>
                                </div>
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <input type="date"
                                    class="px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none"
                                    wire:model.live='perDateStart'>
                                <label for="">To : </label>
                                <input type="date"
                                    class="px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none"
                                    wire:model.live='perDateEnd'>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach ($hostels as $hostel)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <div>
                                                <p class="font-medium text-gray-900">{{ $hostel->slug }}</p>
                                                {{-- <p class="text-sm text-gray-500">{{ $booking->hostel->location }}</p> --}}
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-gray-600">
                                            <div>
                                                <p class="font-medium text-gray-900">{{ $hostel->email }}</p>
                                                <p class="text-sm text-gray-500">{{ $booking->phone ?? 'No Phone' }}
                                                </p>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-gray-600">
                                        ...
                                        <td class="px-6 py-4">
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium text-green-600 bg-green-200">{{ $hostel->status }}</span>
                                        </td>
                    ...
                    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                        <p class="text-sm text-gray-500">Showing
                            {{ $hostels->firstItem() }}-{{ $hostels->lastItem() }} of {{ $hostels->total() }}
                            hostels</p>
                        <div class="flex items-center space-x-2">
                            <button wire:click='previousPage' wire:loading.attr='disabled'
                                @class([
                                    'p-2 rounded-lg border transition-colors',
                                    'border-gray-200 text-gray-400 cursor-not-allowed' => $hostels->onFirstPage(),
                                    'border-gray-200 text-gray-700 hover:bg-gray-50' => !$hostels->onFirstPage(),
                                ]) @disabled($hostels->onFirstPage())>
                            ...
                            @foreach (range(1, $hostels->lastPage()) as $page)
                                @if ($page == 1 || $page == $hostels->lastPage() || abs($page - $hostels->currentPage()) <= 1)
                                    <button wire:click='gotoPage({{ $page }})'
                                        class="w-10 h-10 rounded-lg font-medium transition-colors
                    {{ $hostels->currentPage() == $page
                        ? 'bg-primary text-white'
                        : 'border border-gray-200 text-gray-700 hover:bg-gray-50' }}">{{ $page }}
                                    </button>
                                @elseif ($page == 2 || $page == $hostels->lastPage() - 1)
                            ...
                            <button wire:click="nextPage" wire:loading.attr="disabled" @class([
                                'p-2 rounded-lg border transition-colors',
                                'border-gray-200 text-gray-400 cursor-not-allowed' => !$hostels->hasMorePages(),
                                'border-gray-200 text-gray-700 hover:bg-gray-50' => $hostels->hasMorePages(),
                            ])
                                @disabled(!$hostels->hasMorePages())>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 5l7 7-7 7" />
                                </svg>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl transform transition-all">
                    <div class="flex items-center justify-between p-6 border-b border-gray-100">
                        <h2 id="create-modal-title" class="text-xl font-bold text-gray-900">Add New Hostel</h2>
                        <button
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="room-number" placeholder="e.g., 101"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent invalid:border-red-500  outline-none">
                            </div>
                            <div>
    ...
    <script>
        ...
        document.addEventListener('openCreateModal', function() { // verified
        ...
        [statusModal, viewModal, craeteModal].forEach(modal => {
